    ...
    //--------------------------------------------- COMMAND CALLER METHODS ---------------------------------------------
        ...
        if (command.getName().equalsIgnoreCase("restoreplayer")) {
            if (args.length == 1) {
                ...
                player.sendMessage("§cInvalid command usage. Usage: /restoreplayer <username>");
            }
            return true;
        }
        if (command.getName().equalsIgnoreCase("livesperplayer")) {
            ...
            if (args.length == 1) {
                ...
                player.sendMessage("§cInvalid command usage. Usage: /excludeplayer <username>");
            }
            return true;
        }
            ...
            if (args.length == 1) {
                ...
                player.sendMessage("§cInvalid command usage. Usage: /includeplayer <username>");
            }
            return true;
        }
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    public void excludeplayer (String username) {
        plugin.config.excludedPlayers.add(username);
        plugin.configYaml.set("config.excludedPlayers", plugin.config.excludedPlayers);
        plugin.saveConfigFile();
    }
    public void includeplayer (String username) {
        plugin.config.excludedPlayers.remove(username);
        plugin.configYaml.set("config.excludedPlayers", plugin.config.excludedPlayers);
        plugin.saveConfigFile();
    }
